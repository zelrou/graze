...
export function getXPathForElement(el, xml) {
  ...
  while (el !== xml.documentElement) {
    ...
    xpath = `*[name()='${el.nodeName}' and namespace-uri()='${el.namespaceURI ?? ""
      }'][${pos}]/${xpath}`;

    el = el.parentNode;
  }
  xpath = `/*[name()='${xml.documentElement.nodeName}' and namespace-uri()='${el.namespaceURI ?? ""
    }']/${xpath}`;
  xpath = xpath.replace(/\/$/, "");
  return xpath;
}
...
const resolveNamespace = (clean: Document | Node) => {
  return clean.ownerDocument === null
    ? clean.documentElement
    : clean.ownerDocument.documentElement;
}

const purifyOpts: Config = {
  // Readability needs document object
  RETURN_DOM: true
}

/**
 * These sanitize/articlize versions used for jsdom unit testing
*/
// TODO this is needed in e2e context
export const _cleanNode = (node: WindowLike): [Document, Document] => {
  if (!(node.nodeType === document.DOCUMENT_NODE)) {
    throw new TypeError(`expected node not, ${node}`)
  }
  const clean = isoDomPurify.sanitize(node.documentElement.outerHTML, {
    RETURN_DOM: true,
    ...purifyOpts,
  });
  const nsResolver = resolveNamespace(clean)
  return [clean, nsResolver]
}

export const _sanitizeAndReaderize = (node: WindowLike, opts = purifyOpts) => {
  globalThis.Readability = Readability;
  const [clean, nsResolver] = _cleanNode(node)
  const cleanDoc = clean.ownerDocument
  const { RETURN_DOM } = opts
  const cleanArticle = new Readability(cleanDoc, {
    nbTopCandidates: 15,
    serializer: (el) => RETURN_DOM ? el : el.innerHTML
  }).parse();
  return cleanArticle;
}
...
/*
* Define the sanitze function
*/
export const cleanNode = (node: Document):[HTMLBodyElement, Document] => {
  const nsResolver = resolveNamespace(node)
  // const DOMPurify = createDOMPurify(node);
  // const clean = DOMPurify.sanitize(node.documentElement, {
  //   ...purifyOpts
  // })
  const clean = isoDomPurify.sanitize(node.documentElement.outerHTML, {
    RETURN_DOM: true,
...
export function sanitzeAndReaderize(node: WindowLike, RETURN_DOM = false):
  ...
  if (isProbablyReaderable(document)) {
    ...
    article = new Readability(cleanDoc, {
      nbTopCandidates: 15,
      serializer: (el) => RETURN_DOM ? el : el.innerHTML
    }).parse();
...
* #parsing_an_input_using_trusted_types
*/
const createTrustedTypes = () => {
  if (typeof window.trustedTypes === "undefined")
    window.trustedTypes = { createPolicy: (n, rules) => rules };
  const policy = window.trustedTypes.createPolicy(
    "sanitize-policy", cleanNode);
  return policy
}
export const policy = globalThis.window ? createTrustedTypes() : null;

export const phXPath = [`//title`, `//meta[@name="AUTHOR"]`, `//p`, `//h1`,
...
export function getParagraphsWithHeadings(clean, nsResolver, expr = phXPath) {
