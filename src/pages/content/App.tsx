...
const syncStorageState = changes => {
    //const { pIdx, cIdx } = storageChange;
    console.log('storageChanges:', changes)
...
export default function App() {
    ...
    const handleClickMinimize = e => {
        console.log('handleClickMinimize', e, isMinimized)
    ...
    const handleClickClearBookmarks = async (e) => {
        e.preventDefault()
        browser.storage.local.set({[locationHref]: {}})
    }
    ...
    const classesMainText = [
        'font-[Georgia] text-2xl indent-0 text-left px-20',
    ...
    return (
        ...
        <div id='reader-modal-root' className={ ['z-50 grid',
            'p-4 text-lg text-slate-300 bg-zinc-700 border',
            isMinimized ? 'w-fit' : 'w-7/10',
            isMinimized ? 'h-full' : 'h-7/10',
            ...
            <div className='grid grid-rows-5 h-full'>
                ...
                {/* ========== TOOLBAR TOP ========== */}
                <div className='row-span-1 align-self-start justify-self-center place-items-center grid grid-cols-5'>
                    <div className='col-start-2 col-span-3 place-items-center'>Reader</div>
                    <div className='col-span-1 gap-4 place-items-end grid grid-cols-3 divide-x-3 divide-dashed divide-indigo-500 text-sm'>
                        ...
                        <div className=''>
                            <button className='' onClick={()=>setIsMinimized(!isMinimized)}>
                                {!isMinimized ? '-' : 'O'}
                            </button>
                        </div>
                        <div className=''><button className=''>X</button></div>
                    </div>
                </div>
                ...
                <div className={['row-span-3 grid grid-rows-10',
                    ...
                    <div className='place-self-center row-span-8 h-full w-5/10 border p-5'>
                        <p className={classesMainText}>
                            { mainText }
                        </p>
                    </div>
                    <div className='row-span-1 w-4/10 grid place-self-center place-items-center'>
                        <progress value={charIndex/paragraphLength} />
                        <span className='text-sm text-black fixed'>{`${charIndex}/${paragraphLength}`}</span>
                    </div>
                    <div className='row-span-1 w-4/10 grid place-self-center place-items-center'>
                        <progress color='orange' value={paragraphIndex/paragraphsLength} />
                ...
                <div className={['row-span-1 grid grid-cols-6',
                    ...
                    <div className={'grid grid-rows-3 col-span-2'}>
                        ...
                        <form method="post" onSubmit={handleSubmitSettings}
                            className="w-full flex flex-row justify-self-start place-items-center" >
                            <div className='basis-xs flex flex-row place-items-center'>
                                <label for='paragraph'>¶:
                            ...
                            <div className='basis-xs flex flex-row place-items-center'>
                                <button type="submit"
                                   className='w-full bg-indigo-500 hover:bg-fuchsia-500'>
                                   go </button>
                            </div>
                        </form>

                        {/* Auto Controls */}
                        <div className="w-full flex flex-row justify-self-start place-items-center">
                            <div className='basis-xs grid grid-cols-2'>
                                <button className='border' onClick={handleClickPrev}>back</button>
                                <button className='border' onClick={handleClickNext}>fwrd</button>
                            </div>
                            ...
                            <button className='basis-xs border' onClick={handleClickPause}>{isPaused ? 'play' : 'pause'}</button>
                        </div>
                    </div>

                    {/* ========== Bookmark Toolbar  ========== */}
                    <div className={'col-span-2 col-start-5 grid-cols-1 place-items-center'}>
                        <div className={'grid grid-rows-2 place-items-center'}>
                            <div>
                                <span>Bookmarks</span>
                            </div>
                            <div>
                                <button className='border p-2 hover:bg-yellow-500'
                                    onClick={handleClickSetBookmarkLatest} >
                                    Mark Latest</button>
                                <button className='border p-2 hover:bg-fuchsia-500'
